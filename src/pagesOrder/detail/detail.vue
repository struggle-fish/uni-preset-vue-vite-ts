    ...
    <template v-if="order">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-else> 
          ...
          <view class="button-group">
            ...
            <view v-if="isDev && order.orderState === OrderState.DaiFaHuo" 
            @click="onOrderSend"
            class="button"> 模拟发货 </view>
            <view v-if="order.orderState === OrderState.DaiShouHuo" @click="onOrderConfirm" class="button"> 确认收货 </view>
          </view>
        </template>
      </view>
      <!-- 配送状态 -->
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-else>
          ...
          <view class="button delete" @click="onOrderDelete" v-if="order.orderState >= OrderState.DaiShouHuo"> 删除订单 </view>
        </template>
      </view>
    </template>
    <template v-else>
...
<script setup lang="ts">
  ...
  import { deleteMemberOrderAPI, getMemberOrderByIdAPI, getMemberOrderConsignmentByIdAPI, getMemberOrderLogisticsByIdAPI, putMemberOrderReceiptByIdAPI } from '@/services/oreder';
  ...
  const onOrderConfirm = async() => {
    uni.showModal({
      content: '为保障您的权益，请收到货后确认无误，在确认收货',
      success: async(res)=>{
        const resp = await putMemberOrderReceiptByIdAPI(query.id)
